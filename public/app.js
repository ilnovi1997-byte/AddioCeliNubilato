...
socket.on('update_scoreboard', (users) => {
    ...
    if (me) document.getElementById('userPoints').textContent = `${me.points} Punti`;
});
...
function renderRanking(users) {
    ...
    list.innerHTML = sorted.map(u => `<li><span>${u.name}</span> <strong>${u.points} pt</strong></li>`).join('');
}

function renderFeed(feed) {
    const list = document.getElementById('feedList');
    list.innerHTML = feed.map(p => `<div class="post-item"><strong>${p.user}:</strong> ${p.text} <small>${p.time}</small></div>`).join('');
}
